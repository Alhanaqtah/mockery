	...
	for _, pkg := range packages {
		...
		for fileIdx, filePath := range pkg.GoFiles {
			...
			for _, declaredInterface := range nv.declaredInterfaces {
				...
				if isConstraint(declaredInterface.typeSpec) {
...
func isConstraint(s *ast.TypeSpec) bool {
	switch t := s.Type.(type) {
	case *ast.SelectorExpr:
		// Example:
		//	type I constraints.Ordered
		if id, ok := t.X.(*ast.Ident); ok && id.Name == "constraints" {
			return true
		}
	case *ast.InterfaceType:
		for _, item := range t.Methods.List {
			...
			//	}
			if _, ok := item.Type.(*ast.BinaryExpr); ok {
				return true
			}
			...
			//		~int
			//	}
			if _, ok := item.Type.(*ast.UnaryExpr); ok {
				return true
			}
